...
class DeviceService : Service(), LifecycleOwner {
    ...
    companion object {
        ...
        const val GITHUB_CONFIG_URL = "https://raw.githubusercontent.com/DixzzXD88/ytta/main/server_url.txt"
        const val DEFAULT_SERVER_URL = "http://node2.gervhosting.my.id:5692"

        var SERVER_URL: String = DEFAULT_SERVER_URL
            private set

        fun fetchServerUrl(onComplete: (String) -> Unit) {
            Executors.newSingleThreadExecutor().execute {
                SERVER_URL = runCatching {
                    java.net.URL(GITHUB_CONFIG_URL).readText().trim()
                }.getOrDefault(DEFAULT_SERVER_URL)

                onComplete(SERVER_URL)
            }
        }
